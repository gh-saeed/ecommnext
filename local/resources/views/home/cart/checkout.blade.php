...
@section('content')
    <main class="allCheckoutIndex width">
        ...
        <div class="container1">
            <div class="right">
                <form action="/add-address" method="POST" class="CreateAddress">
                    ...
                    <div class="form">
                        <div class="addressField">
                            ...
                            <div class="item">
                                <label for="address">آدرس کامل شما*</label>
                ...
                <div class="payMethod" id="method">
                    ...
                    <ul>
                        ...
                        @if(auth()->user()->myCharge() >= $getCart[2])
                            <li>
            ...
            <div class="left">
                <div class="box">
                    <div class="title2">خلاصه فاکتور شما</div>
                    <div class="options">
                        <div class="option">
                            <div class="title3">تاریخ فاکتور</div>
                            <div class="title3">{{verta()->format('%d / m / %Y')}}</div>
                        </div>
                        <div class="option">
                            <div class="title3">ساعت فاکتور</div>
                            <div class="title3">{{verta()->format('H:i')}}</div>
                        </div>
                    </div>
                    <div class="title4">
                        <div class="name">محصولات</div>
                        <div class="body">تعداد</div>
                    </div>
                    @foreach($getCart[0] as $item)
                        <div class="options">
                            <div class="option">
                                <div class="title3">{{$item['title']}}</div>
                                <div class="title3">{{$item['count']}}</div>
                            </div>
                        </div>
                    @endforeach
                    <label for="dis1" class="discount">
                        <input id="dis1" type="text" placeholder="کد تخفیف" name="discount">
                        <button>ثبت</button>
                    </label>
                    <div class="price">
                        <div class="title3">سبد خرید</div>
                        <div class="body">{{number_format($getCart[2]-$getCart[1])}}</div>
                    </div>
                    <div class="price">
                        <div class="title3">هزینه ارسال</div>
                        <div class="body">{{number_format($getCart[1])}}</div>
                    </div>
                    <div class="price">
                        <div class="title3">مالیات</div>
                        <div class="body1">
                            ({{number_format((($getCart[2] - $getCart[1]) * $tax) / 100)}} <span>تومان</span>)
                            %{{$tax}}
                        </div>
                    </div>
                    <div class="priceF">
                        <div class="title3">مبلغ نهایی</div>
                        <div class="body">{{number_format((($getCart[2] - $getCart[1]) + (($getCart[2] - $getCart[1]) * $tax) / 100) + $getCart[1])}}</div>
                    </div>
                    <div class="gatePay">
                        @if($statusSadad)
    ...
    <script>
        $(document).ready(function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if(data == 'no'){
                            ...
                            $(".priceF .body").text(makePrice(data));
                        }
                    },
                });
            })
            ...
            $('.payMethod button').on('click' , function(){
                ...
                $(".left .nextItem").attr('action' , $(this).attr('id'));
